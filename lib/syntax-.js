...
function tokenizeTable(effects, ok, nok) {
  ...
  function startXTable(code) {
    return effects.attempt([{
      tokenize: tokenizeFromHead,
      partial: true
    }, {
      tokenize: tokenizeFromDelimiter,
      partial: true
    }], function (code) {
      console.log('====ok'); // @ts-ignore

      effects._inFullTable = false;
      return ok(code);
    })(code);
  }
  /* XTABLE */
  ...
  function tokenizeFromHead(_effects, _ok, _nok) {
    // @ts-ignore
    console.log('<<<<<<<<<< launching full: ' + _effects._inFullTable);
    /* Need to store ok/nok from parent object */

    effects = _effects;
    ok = _ok;
    nok = _nok; // @ts-expect-error Custom.

    effects._inFullTable = true;
    return start;
  }
  /* XTABLE */
  ...
  function tokenizeFromDelimiter(_effects, _ok, _nok) {
    // @ts-ignore
    console.log('>>>>>>>> launching headerless: ' + _effects._inFullTable);
    /* Need to store ok/nok from parent object */
  ...
  function startDelimiter(code) {
    /* Headerless table has to start with pipe */
    if (code !== 124) {
      return nok(code);
